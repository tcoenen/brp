...
from brp.svg.plotters.gradient import BWGradient, RGBGradient
from brp.core.bbox import find_bounding_box, combine_bbox
...
def bin_data_2d(x_seq, y_seq, x_bins, y_bins, hist_bbox):
    ...
    for x, y in zip(x_seq, y_seq):
        if not (hist_bbox[0] <= x < hist_bbox[2]): continue
        if not hist_bbox[1] <= y < hist_bbox[3]: continue
...
class Histogram2dPlotter(RasterPlotterMixin):
    def __init__(self, x_seq, y_seq, *args, **kwargs):
        ...
        gradient = kwargs.get('gradient', 
            RGBGradient((min_val, max_val), (0, 0, 1), (1, 0, 0)))
